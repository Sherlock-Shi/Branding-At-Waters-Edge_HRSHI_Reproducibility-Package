...
2. Annotation: `code/src/llm_scorer.py` submits sentences to the OpenAI Batch API and requires `OPENAI_API_KEY` in the environment. The study used model `gpt-5.1-2025-11-13` at temperature 0 with a 100-token cap and JSON object response format. The expected output is 37,312 annotated sentences.
...
- `data/aggregated/article_level_scores.csv` — 14,564 rows — article-level party, weight, scores for General Ukraine Aid Stance (q1), Ukraine Characterization (q2), Threat Priority (q3), and Resource Allocation (q4), and counts.
...
- `data/scored/factiva_llmrated_scores.csv` — 37,312 rows — stable IDs, title and body key, sentence metadata, article weights, scores for General Ukraine Aid Stance (q1), Ukraine Characterization (q2), Threat Priority (q3), and Resource Allocation (q4), error type, and batch ID.
- `data/search_string.txt` — 3 lines — the Factiva query and retrieval note.
- `data/statistics/STATISTICAL_TESTS_README.md` — 106 lines — the statistical analysis specification.
- `data/statistics/acf_pacf_q1.png` — N/A rows; 2,970 × 2,365 pixels — General Ukraine Aid Stance autocorrelation and partial-autocorrelation plot.
...
- `data/statistics/ar2_residual_acf_pacf_q1.png` — N/A rows; 2,970 × 2,365 pixels — General Ukraine Aid Stance AR(2) residual diagnostics.
- `data/statistics/articles_with_polarization_q1.png` — N/A rows; 4,224 × 2,294 pixels — General Ukraine Aid Stance article counts and polarization.
- `data/statistics/breakpoint_analysis_metadata.json` — N/A rows; 34 top-level fields — shift-analysis parameters, counts, and diagnostics.
...
- `data/statistics/breakpoint_models.csv` — 4 rows — shift model specifications, fit criteria, diagnostics, and coefficients.
- `data/statistics/breakpoint_regimes.csv` — 12 rows — shift-regime onsets, durations, means, and adjacent shifts.
...
- `data/statistics/tables/table_s3a_trend_break_model_comparison.csv` — 12 rows — trend and shift model comparisons.
- `data/statistics/tables/table_s3b_trend_break_coefficients.csv` — 44 rows — trend-shift model coefficients and intervals.
...
- `data/statistics/tables/table_s4a_breakpoint_duration_sensitivity.csv` — 8 rows — minimum-regime-duration sensitivity and optimizer recovery.
- `data/statistics/tables/table_s4b_breakpoint_regimes.csv` — 12 rows — reported shift regimes, means, and shifts.
...
- `data/statistics/dominance_ready.csv` — 34 rows — monthly General Ukraine Aid Stance, Ukraine Characterization, Threat Priority, and Resource Allocation polarization series.
...
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_fitted_series.csv` — 272 rows — joint AR-shift fitted series.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_focused_candidates.csv` — 5 rows — focused joint AR-shift candidates.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_focused_fitted_series.csv` — 170 rows — focused-candidate fitted series.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_metadata.json` — N/A rows; 26 top-level fields — joint AR-shift parameters, counts, and diagnostics.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_model_diagnostics.csv` — 8 rows — joint AR-shift model diagnostics.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_party_brand_index_diagnostics.png` — N/A rows; 5,821 × 3,352 pixels — Party Brand Index shift diagnostics.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_party_brand_index_focused.png` — N/A rows; 3,392 × 3,352 pixels — focused Party Brand Index shift comparison.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_polarization_diagnostics.png` — N/A rows; 5,821 × 3,352 pixels — polarization shift diagnostics.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_polarization_focused.png` — N/A rows; 4,982 × 3,352 pixels — focused polarization shift comparison.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_regime_statistics.csv` — 24 rows — joint AR-shift regime statistics.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_search_convergence.csv` — 20 rows — shift-search convergence frequencies.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_search_runs.csv` — 80 rows — shift-search runs, seeds, configurations, and fit values.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_stability.png` — N/A rows; 4,954 × 2,680 pixels — shift-search stability.
- `data/statistics/joint_ar_breakpoints/joint_ar_bic_winners.csv` — 8 rows — winning shift configurations and recovery rates.
- `data/statistics/mann_kendall_s_decomposition.csv` — 3 rows — positive, negative, tie, and S contributions.
- `data/statistics/measurement_bootstrap_metadata.json` — N/A rows; 17 top-level fields — article-cluster bootstrap parameters, counts, and diagnostics.
- `data/statistics/monthly_message_coherence.csv` — 68 rows — monthly within-party coherence, intervals, counts, and valid replicates.
...
- `data/statistics/party_brand_index_timeseries.csv` — 34 rows — monthly polarization and within-party coherence components, Party Brand Index, intervals, and counts.
...
- `data/statistics/q1_stance_mmk_trend.png` — N/A rows; 4,233 × 2,295 pixels — General Ukraine Aid Stance trend.
...
- `data/statistics/temporal_diagnostics/party_brand_index_measurement_and_breakpoint.png` — N/A rows; 2,304 × 1,473 pixels — Party Brand Index measurement and shift diagnostic.
- `data/statistics/temporal_diagnostics/q1_polarization_measurement_and_breakpoint.png` — N/A rows; 2,304 × 1,473 pixels — General Ukraine Aid Stance polarization measurement and shift diagnostic.
- `data/statistics/trend_break_analysis_metadata.json` — N/A rows; 19 top-level fields — trend-shift analysis parameters, counts, and diagnostics.
...
- `data/statistics/trend_break_model_comparison.csv` — 12 rows — trend-shift model comparisons by minimum regime duration.
...
- `data/validation/manual_coding_BLIND.csv` — 50 rows — blind coding worksheet with two coders' General Ukraine Aid Stance (q1), Ukraine Characterization (q2), Threat Priority (q3), and Resource Allocation (q4) fields.
...
- `data/validation/test6_manual_results.csv` — 50 rows — parsed coder and LLM General Ukraine Aid Stance (q1), Ukraine Characterization (q2), Threat Priority (q3), and Resource Allocation (q4) results with party and date.
